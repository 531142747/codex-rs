...
const WINDOWS_GIT_BASH_PATH: &str = r"C:\Program Files\Git\usr\bin\bash.exe";
...
fn default_user_shell_from_path(user_shell_path: Option<PathBuf>) -> Shell {
    if cfg!(windows) {
        let git_bash_path = PathBuf::from(WINDOWS_GIT_BASH_PATH);
        if let Some(shell_path) = file_exists(&git_bash_path) {
            return Shell {
                shell_type: ShellType::Bash,
                shell_path,
                shell_snapshot: empty_shell_snapshot_receiver(),
...
mod tests {
    ...
    #[tokio::test]
    async fn detects_default_shell_on_windows() {
        if !cfg!(windows) {
            return;
        }
        ...
        let git_bash_path = PathBuf::from(WINDOWS_GIT_BASH_PATH);
        if git_bash_path.is_file() {
            assert_eq!(default_shell.shell_type, ShellType::Bash);
            assert_eq!(default_shell.shell_path, git_bash_path);
        } else {
            assert_eq!(default_shell.shell_type, ShellType::PowerShell);
            assert!(
                default_shell.shell_path.ends_with("pwsh.exe")
                    || default_shell.shell_path.ends_with("powershell.exe")
            );
        }
    }
